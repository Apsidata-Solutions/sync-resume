...
from src.utils.defs import ROLES, LEVELS, SKILLS, ROLE_MAPPING, LEVEL_MAPPING, SKILL_MAPPING
from src.utils.helper import sanitise_mobile_number, sanitise_email, is_valid_mobile, is_valid_email
...
class SkillMatcher:
    ...
    def _vector_similarity_match(self, input_text, is_role=False):
        ...
        try:
            ...
            logger.error(f"Vector similarity matching error: {e}")
...
def test_sample(file_path, sample_size=100, strategy='progressive'):
    ...
    try:
        ...
        if 'Primary Skill' not in df.columns:
            logger.error("Error: Column 'Primary Skill' not found in the Excel sheet.")
            return None
        ...
        logger.info(f"Processing sample of {len(sample)} rows...")
...
def run_full_processing(file_path, output_path=None, strategy='progressive', num_workers=None):
    ...
    try:
        ...
        if result is None:
            logger.error("Error: No results returned from processing.")
            return None, None
...
def preprocess_data(given_df, invert = False):
    df = given_df.copy()
    df.rename(
        columns={
            "Id":"id", #Copy Over
            ...
            "LastName": "last_name", 
            "MobileNo": "mobile", 
            "WhatsAppNo":"whatsapp", #Additional field
            "Email": "email", 
            "DOB":"date_of_birth",
            "CountryId":"country_id", #To remove
            "Country":"country",
            "StateId":"state_id", #To remove
            "State":"state",
            "City":"city",
            "Address":"address",
            "Pin":"pin_code",
            "Primary Skill": "old_skills", #To remove
            "Skill": "primary_skill",
            "Role": "role", 
            "Level": "level", 
            "Resume":"resume", # Copy over
        }, inplace=True
    )
    df["prefix"]= None
    df["gender"]= None
    df["alternative_mobile"]= None
    df["alternative_email"]= None
    df["industry"]= "Education"
    df["secondary_skill"]= None
    df["tertiary_skill"]= None
    df["career_start_date"]= None
    df["education"]= None
    df["experiences"]= None

    # Sanitize mobile numbers
    if 'mobile' in df.columns:
        df['mobile'] = df['mobile'].apply(sanitise_mobile_number)
    
    # Sanitize WhatsApp numbers if they exist
    if 'whatsapp' in df.columns:
        df['whatsapp'] = df['whatsapp'].apply(sanitise_mobile_number)

    if "email" in df.columns:
        df["email"] = df["email"].apply(sanitise_email)
    
    # df.head()
    skill_filter = df['primary_skill']!="Unknown"
    number_filter = df["mobile"].apply(is_valid_mobile)
    email_filter = df["email"].apply(is_valid_email)
    filter = skill_filter & number_filter & email_filter
    df["status"] = 0 # 0 is for pending 
    df.loc[filter,"status"] = 3 # 3 is for cancelled (not needed)
    if invert:
        return df[filter]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Process candidate skills')
    input_file = parser.add_argument('--input', type=str, help='Path to the input Excel file')
    output_file = parser.add_argument('--output', type=str, help='Path to the output Excel file')
    args = parser.parse_args()

    result = process_candidate_skills(args.input, mode='full', output_path=args.output)
