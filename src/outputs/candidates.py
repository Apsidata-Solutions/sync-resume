...
def read_master(engine)->tuple:
    ...
    cityEnum = Enum("City", {name:name for name in cities["name"].to_list()})
    stateEnum = Enum("State", {name:name for name in states["name"].to_list()})

    return roleEnum, skillEnum, levelEnum, cityEnum, stateEnum

roleEnum, skillEnum, levelEnum, cityEnum, stateEnum = read_master(engine)
...
class Location(BaseModel):
    ...
    #TODO: The two enums are implemented, but still failing since Enum lists are too long. Limit at 500 options.
    # city: cityEnum = Field(description="The city where the candidate resides.")
    city: str = Field(..., description="The city where the candidate resides.")
    state: stateEnum = Field(description="The state or region where the candidate resides.")
...
class Education(BaseModel):
    ...
    """
    specialization: str = Field(description="The major or field of study of the candidate's degree.")
    school: str = Field(description="The name of the school or college where the degree was earned.")
    university: str = Field(description="The name of the university that granted/will grant the degree. For example, Delhi University would be the university even if the degree was earned at Miranda House College.")
    ...
    start_date: str = Field(..., description="The start date of the candidate's education (format: MM-YYYY).")
    end_date: Optional[str] = Field(None, description="The grant date of the candidate's degree/education certificate (format: MM-YYYY). None if ongoing")
    status: Optional[Literal["completed", "in progress", "dropped out"]] = Field(description="The status of the candidate's education (e.g., completed, in progress, etc.).")
    ...
    def validate_start_date(cls, v):
        try:
            ...
            raise ValueError('Date must be in MM-YYYY format (e.g., 03-2024) or None')
...
class ExperienceItem(BaseModel):
    ...
    skills_applied: skillEnum = Field(..., description="A list of skills that were applied or enhanced during this experience.")
...
class WorkExperience(BaseModel):
    ...
    designation: str = Field(..., description="The job title or position held during this experience.")
    start_date: str = Field(..., description="The start date of this experience (format: MM-YYYY).")
    end_date: Optional[str] = Field(None, description="The end date of this experience (format: MM-YYYY). None if ongoing")
    ...
    current_job_or_not : bool = Field(..., description="Whether this is the current job of the candidate or not. If yes, then end date should be 'None'")
...
class BaseCandidate(Location, ContactInfo, PersonalDisclosure, PersonalInfo, BaseModel):
    ...
    experiences:Optional[List[WorkExperience]] = Field(default_factory=list, description="A comprehensive list of the candidate's professional experiences")
